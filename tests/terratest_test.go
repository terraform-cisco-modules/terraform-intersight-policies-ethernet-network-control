...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-eth-ctrl-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Ethernet Network Control Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/fabric/EthNetworkControlPolicies/%s", moid), expectedJSONTemplate, vars)
}
